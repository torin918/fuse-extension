...
export const HomeShowToken = ({ goto, token }: { goto: GotoFunction; token: TokenInfo }) => {
    return match_combined_token_info(token.info, {
        ic: (ic) => <ShowTokenIc goto={goto} token={token} ic={ic} />,
    });
};

const ShowTokenIc = ({ goto, token, ic }: { goto: GotoFunction; token: TokenInfo; ic: IcTokenInfo }) => {
