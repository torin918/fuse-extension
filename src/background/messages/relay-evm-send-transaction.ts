...
import type { ApproveEvmSendTransactionAction, EvmTransaction } from '~types/actions/approve/evm/send-transaction';
...
export type ResponseBody = MessageResult<string, string>; // Returns transaction hash on success, error message on failure
...
const handler: PlasmoMessaging.MessageHandler<RequestBody, ResponseBody> = async (req, res) => {
    ...
    try {
        ...
        const sendTxAction: ApproveEvmSendTransactionAction = {
        ...
        action = { approve_evm_send_transaction: sendTxAction };
        ...
        const response = await new Promise<ResponseBody>((resolve) => {
            ...
            const interval_id = setInterval(() => {
                (async () => {
                    ...
                    // Open window if not already open and hasn't been shown yet
