...
let cached_recent_addresses: RecentAddresses = [];

// recent addresses ->  // * local secure
export const useRecentAddressesInner = (
    storage: SecureStorage | undefined,
): [RecentAddresses | undefined, (value: RecentAddresses) => Promise<void>] => {
    const [recent_addresses, setRecentAddresses] = useState<RecentAddresses | undefined>(cached_recent_addresses); // use cached value to init

    // watch this key, cloud notice other hook of this
    useEffect(() => {
        if (!storage) return;
        const callback: StorageWatchCallback = (d) => {
            const recent_addresses = d.newValue ?? [];
    ...
    // init on this hook
    useEffect(() => {
        if (!storage) return setRecentAddresses(undefined); // reset if locked
